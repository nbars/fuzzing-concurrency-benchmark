...
class ReadelfPersistantBuilder(AflTargetBuildere):
    ...
    def build(self, purge: bool = False) -> bool:
        ...
        patch = self.get_patches_dir() / "readelf-persistant.patch"

        subprocess.check_call(
            f"git apply {patch.as_posix()}", cwd=binutils_src_dir, shell=True
        )
